...
**Tidak ada state-machine di endpoint ini** — lihat Catatan Desain & Keputusan Teknis Kunci #7.
...
- `outstanding_total` **TIDAK dibatasi period** (posisi saldo sekarang, bukan arus kas periode) dan **bisa negatif** kalau ada invoice yang di-PATCH manual balik ke status `sent`/`overdue` setelah sempat lunas (lihat Catatan Desain #7 soal `PATCH /invoices/{id}/status` tanpa state-machine) — sengaja tidak di-clamp ke 0.
